...
def generateBezier(points, parameters, leftTangent, rightTangent):
    ...
    for i, (point, u) in enumerate(zip(points, parameters)):
        ...
        tmp = point - mybezier.q([points[0], points[0], points[-1], points[-1]], u)
...
def newtonRaphsonRootFind(bez, point, u):
    ...
    """
    d = mybezier.q(bez, u)-point
    numerator = (d * mybezier.qprime(bez, u)).sum()
    denominator = (mybezier.qprime(bez, u)**2 + d * mybezier.qprimeprime(bez, u)).sum()
...
def computeMaxError(points, bez, parameters):
    ...
    for i, (point, u) in enumerate(zip(points, parameters)):
        dist = linalg.norm(mybezier.q(bez, u)-point)**2
        if dist > maxDist:
            maxDist = dist
            splitPoint = i
